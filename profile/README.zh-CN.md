...
<table><tr><td width="72%">
Aria2t 为 aria2 下载引擎提供了一个真正的界面，既在终端里，也在浏览器里，而且在你关掉其中任何一个之后，队列仍会继续运行。
...
这些承诺适用于我们现在和今后发布的每一款产品。

- **你的数据属于你。** 你可以用易读的格式导出数据。我们不会用这些数据建立你的用户画像。
- **我们的产品没有广告。** 我们不投放广告，也不会收钱把他人的利益置于你的利益之上。
- **我们会说明产品如何工作。** 文档会用清楚的语言说明所有可能影响你信任的功能。
- **没有我们，产品也能继续运行。** 如果我们停止维护，已安装的产品仍会继续运行。我们会提前通知支持结束的时间。

## ✉️ 给我们写信

每封发到 [hello@c0nn3ct.info](mailto:hello@c0nn3ct.info) 的邮件我们都会读。你可以提出想法、报告问题，或者参与产品开发。

无需填写表单，也没有工单号：邮件会直接发给我们。
